...
export const CertLogicGeekyRendering = ({ expr }: { expr: CertLogicExpression }) => {
        ...
        return <div className="array">
            ...
            {(expr as CertLogicExpression[]).map((subExpr, index) =>
                <div className="inline" key={index}>
                    ...
                    <CertLogicGeekyRendering expr={subExpr} key={index} />
                </div>
            )}
        ...
        switch (operator) {
            case "and": {
                if (values.length === 2) {
                    return <div className="operation">
                        <CertLogicGeekyRendering expr={values[0]} />
                        <span className="keyword push-both">and</span>
                        <CertLogicGeekyRendering expr={values[1]} />
                    </div>
                }
                return <div className="operation">
                    <span className="variadic-operator">and</span>
                    <div className="indent">
                        {values.map((subExpr: CertLogicExpression, index: number) => <div key={index}><CertLogicGeekyRendering expr={subExpr} /></div>)}
                    </div>
                </div>
            }
            case "===":
            ...
            case "if": {
                ...
                return <div className="operation">
                    ...
                    <CertLogicGeekyRendering expr={guard} />
                    <span className="keyword push-both">then</span>
                    <CertLogicGeekyRendering expr={then} />
                    <span className="keyword push-both">else</span>
                    <CertLogicGeekyRendering expr={else_} />
                </div>
            }
            case "plusTime": {
                const [ operand, amount, unit ] = values
                return <div className="operation">
                    <CertLogicGeekyRendering expr={operand} />
            ...
            case "reduce": {
                return <div className="operation">
                    ...
                    <CertLogicGeekyRendering expr={values[2]} />
